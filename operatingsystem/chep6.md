...
- 定义
数据项是有结构文件的基本单位，是数据组织中可以命名的最小逻辑单位。也称字段。每个数据项包括：`名称、数据类型、数据宽度`等。
- 分类
  - 基本数据项：单个数据项；
  - 组合数据项：若干个数据项集；
- 实例: 姓名，性别。
...
- 单级索引：\
索引表的存储空间假设磁盘块尺寸为4KB，磁盘块号为32bits。则每个磁盘块可以存储磁盘块号的数量为：\
```4K/4 = 1K ；```\
而单级索引支持的文件尺寸为：\
```1K * 4K = 4 M```。\
即小于4M文件的索引表只占1磁盘块。
...
- 二级索引：\
索引表的存储空间假设磁盘块尺寸为4KB，磁盘块号为32bits。则每个磁盘块可以存储磁盘块号：\
```4K/4 = 1K （个）；```\
而二级索引支持的文件尺寸为：\
```1K * 1K * 4K = 4G。```\
索引表占用空间：\
